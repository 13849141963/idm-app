...
var vm = new Vue({
	...
	methods: {
        ...
        saveUserData: function (event) {
			...
			//添加权限集,先获取用户的userId,在查询所有权限集
            location.href = "baseUserData.html?userId="+userId;
        },
		//查看权限集
        queryUserData: function (event) {
            var userId = getSelectedRow();
            if(userId == null){
                return ;
            }
            location.href = "baseUserData_userAll.html?userId="+userId;
        },
