$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{label: '类型', name: 'orgType', width: 60,formatter: function(value, options, row){
                    return value == 2 ?
                        '<span class="label label-danger">部门</span>' :
                        '<span class="label label-success">机构/公司</span>';
                } },
        ],
		viewrecords: true,
        height: 400,
...
function reloadGrid(){
    var parm = $("#orgName").val();
    var sys = vm.selected1;
    //parm = encodeURI(parm);
    if(parm != "undefined" || sys != "undefined")
        $("#jqGrid").jqGrid('setGridParam', {postData: {orgName:parm}}).trigger("reloadGrid");
}